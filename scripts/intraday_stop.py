...
CIRCUIT_BREAK = 3        # 连续止损N次触发熔断
CIRCUIT_DAYS = 5          # 熔断空仓天数
LIMIT_DOWN_PCT = -0.08    # 跌停线 -8%
...
def get_price_info(code):
    """Returns (open_price, now_price, yesterday_close) or (None,None,None)."""
    try:
        ...
        return float(f[1]), float(f[3]), float(f[2])  # open, now, last_close
...
def run_screener(exclude_codes, capital):
    ...
    for c in data.get("candidates", []):
        ...
        if "ST" in c["name"]: continue
        price = c["price"]
        amount = capital * 0.125
...
def is_in_circuit(state, strategy):
...
def main():
    ...
    for strategy in ["烽火V5", "5-Gate"]:
        if not state[strategy]:
            for code, name, cost, stop, shares, buy_date in INITIAL[strategy]:
                state[strategy].append({
                    "code": code, "name": name, "cost": cost, "stop": stop,
                    "shares": shares, "buy_date": buy_date
                })
        state.setdefault("cash", {}).setdefault(strategy, 0)
    
    for strategy in ["烽火V5", "5-Gate"]:
        if is_in_circuit(state, strategy):
            until = state["circuit_until"][strategy]
            alerts.append(f"⛔ {strategy} 熔断中 恢复日: {until}")
            continue
        ...
        for pos in state[strategy]:
            ...
            # === V2: 跌停保护 ===
            limit_down_hit = open_p and last_close and (open_p / last_close - 1) < LIMIT_DOWN_PCT
            
            if limit_down_hit and can_sell:
                # 开盘跳空大跌 → 立刻挂 open 价卖出
                cash = pos["shares"] * open_p * 0.9975
                alerts.append(f"💀 {strategy} {code} {pos.get('name','')} 跌停低开! 开盘价{open_p:.2f} 回收¥{cash:,.0f}")
                sold_cash[strategy] += cash
                new_stops[strategy] += 1
            ...
            # === 正常止损 ===
            if now_p <= pos["stop"] and can_sell:
                cash = pos["shares"] * now_p * 0.9975
                pnl = (now_p - pos["cost"]) / pos["cost"] * 100
                alerts.append(f"🚨 {strategy} {code} {pos.get('name','')} ¥{now_p:.2f} 触发止损 回收¥{cash:,.0f}")
                sold_cash[strategy] += cash
                new_stops[strategy] += 1
                save_trade({"date":today,"strategy":strategy,"action":"卖出","code":code,
                ...
                alerts.append(f"🔒 {strategy} {code} T+1锁定")
                remaining.append(pos)
            elif pnl < -3:
                alerts.append(f"⚠️ {strategy} {code} ¥{now_p:.2f} 接近止损 {pnl:+.1f}%")
                remaining.append(pos)
            else:
                remaining.append(pos)
    ...
    # === V2: 熔断检查 ===
    for strategy in ["烽火V5", "5-Gate"]:
        if is_in_circuit(state, strategy): continue
        
        cb = state["circuit_breaker"].get(strategy, 0)
        cb += new_stops[strategy]
        ...
        if cb >= CIRCUIT_BREAK:
            ...
            for pos in state[strategy]:
                ...
                if now_p:
                    extra_cash += pos["shares"] * now_p * 0.9975
                    alerts.append(f"⛔ 熔断清仓 {strategy} {pos['code']} {pos.get('name','')} ¥{now_p:.2f}")
            state[strategy] = []
            state["cash"][strategy] += extra_cash
            state["circuit_breaker"][strategy] = 0
            circuit_end = (today + timedelta(days=CIRCUIT_DAYS)).isoformat()
            state["circuit_until"][strategy] = circuit_end
            alerts.append(f"⛔ {strategy} 触发熔断! 连续{CIRCUIT_BREAK}次止损，清仓暂停至 {circuit_end}")
        else:
    ...
    for strategy in ["烽火V5", "5-Gate"]:
        ...
        if picks:
            ...
            for code, name, price, shares, cost, score in picks:
                ...
                state[strategy].append({
                    "code": code, "name": name, "cost": price * 1.0025, "stop": price * 0.95,
                    "shares": shares, "buy_date": today
                })
                state["cash"][strategy] -= cost
                save_trade({"date":today,"strategy":strategy,"action":"买入","code":code,
                    "name":name,"price":round(price,2),"shares":shares,
                    "amount":round(cost,0),"reason":"补仓"})
